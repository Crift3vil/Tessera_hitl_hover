...
double current_ros_target = 2.0; // Default 2 meters
bool first_run = true;
double alt_offset = 0.0;

// ROS 2 Callback: Updates target whenever a message arrives
void onRosTarget(const std_msgs::msg::Float64::SharedPtr msg) {
    current_ros_target = msg->data;
}

// Gazebo Callback: The high-speed loop
void onAltitudeReceived(const gz::msgs::Altimeter &_msg) {
    ...
    // 1. Declare the variable at the top of the function
    ...
    // 3. Now the print statement is OUTSIDE the try block and can see 'final_thrust'
